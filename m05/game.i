...
void draw(){
 ...
 drawFlowers();
 (*(volatile unsigned short *)0x04000014) = (hOffClouds/3);
...
void drawLives() {
 for(int i = 0; i < 3; i++){
  ...
  if(lives[i].active){
   shadowOAM[i + 2].attr0 = (0xFF & lives[i].row) | (0<<13) | (0<<14);
   shadowOAM[i + 2].attr1 = (0x1FF & lives[i].col) | (1<<14);
   shadowOAM[i + 2].attr2 = ((0)<<12) | ((0)*32+(0));
  }
  else {
...
void drawWaterTracker(){
 shadowOAM[1].attr0 = (0xFF & 0) | (0<<13) | (2<<14);
 shadowOAM[1].attr1 = (0x1FF & 0) | (3<<14);
 shadowOAM[1].attr2 = ((0)<<12) | ((0)*32+(28));
...
void updateWaterTracker(){
 ...
 for (int i = 0; i < 50; i++){
  if (tracker[i].id <= fill){
   tracker[i].active = 1;
   tracker[i].row = 54 - i;
   shadowOAM[ i + 30].attr0 = (0xFF & tracker[i].row) | (0<<13) | (1<<14);
   shadowOAM[ i + 30].attr1 = (0x1FF & tracker[i].col) | (0<<14);
   shadowOAM[ i + 30].attr2 = ((0)<<12) | ((0)*32+(3));
  } else {
   tracker[i].active = 0;
   shadowOAM[i + 30].attr0 = (2<<8);
  }
 }
}
...
void updatePlayer(){
 ...
 if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<0))) && player.dropTimer >= 10 && player.holding >= 5) {
  player.pouring = 1;
  fireDrops();
  player.dropTimer = 0;
  if (!player.cheat){
 ...
 if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<1))) && player.dropTimer >= 10 && player.col > 440) {
  fireDropsFaucet();
  player.dropTimer = 0;
 }
...
void drawPlayer() {
 if (!player.cheat){
  shadowOAM[0].attr0 = (0xFF & player.screenRow) | (0<<13) | (0<<14);
     ...
     shadowOAM[0].attr2 = ((2 + (player.pouring * 4))*32+(0 + (player.flipped * 4))) | ((0)<<12);
    }else{
     shadowOAM[0].attr0 = (0xFF & player.screenRow) | (0<<13) | (0<<14);
     shadowOAM[0].attr1 = (0x1FF & player.screenCol) | (2<<14);
     shadowOAM[0].attr2 = ((2 + (player.pouring * 4))*32+(0 + (player.flipped * 4) + (8 * player.cheat))) | ((1)<<12);
    }
 ...
 for(int i = 0; i < 9; i++){
  if(flowers[i].active == 1){
   if (flowers[i].stage == 1){
    shadowOAM[i + 5].attr0 = (0xFF & flowers[i].row) | (0<<13) | (0<<14);
    shadowOAM[i + 5].attr1 = (0x1FF & flowers[i].col) | (0<<14);
    shadowOAM[i + 5].attr2 = ((0)<<12) | ((10 + (flowers[i].wilt * 11))*32+(0 + (flowers[i].aniState * 4)));
   } else if (flowers[i].stage == 2){
    shadowOAM[i + 5].attr0 = (0xFF & flowers[i].row) | (0<<13) | (0<<14);
    shadowOAM[i + 5].attr1 = (0x1FF & flowers[i].col) | (1<<14);
    shadowOAM[i + 5].attr2 = ((0)<<12) | ((11 + (flowers[i].wilt * 11))*32+(0 + (flowers[i].aniState * 4)));
   }
   else if (flowers[i].stage == 3){
    shadowOAM[i + 5].attr0 = (0xFF & flowers[i].row) | (0<<13) | (0<<14);
    shadowOAM[i + 5].attr1 = (0x1FF & flowers[i].col) | (2<<14);
    shadowOAM[i + 5].attr2 = ((0)<<12) | ((13 + (flowers[i].wilt * 11))*32+(0 + (flowers[i].aniState * 4)));
   }
   else if (flowers[i].stage == 4){
    shadowOAM[i + 5].attr0 = (0xFF & flowers[i].row) | (0<<13) | (0<<14);
    shadowOAM[i + 5].attr1 = (0x1FF & flowers[i].col) | (2<<14);
    shadowOAM[i + 5].attr2 = ((0)<<12) | ((17 + (flowers[i].wilt * 11))*32+(0 + (flowers[i].aniState * 4)));
   }
  }
  else {
 ...
 if (f->active == 1){
  ...
  if (f->holding <= 20) {
   ...
   if (f->oldHolding > 20) {
    f->wilt = 1;
    f->done = 0;
    playSoundB(wilt_c2, 17770, 11025, 0);
   }
   ...
   if (f->oldHolding <= 0){
    ...
    livesRemaining--;
    flowersAlive--;
    playSoundB(dead_c2, 28307, 11025, 0);
   }
  } else {
   ...
   if (f->holding >= f->capacity - f->decayRate){
    ...
    if (f->stage == 4 && (!f->done)){
     f->done = 1;
     playSoundB(sparkle_c2, 24801, 11025, 0);
     growNewFlower();
